...
[^1]: $$\hbar$$, $$e$$, $$m_e$$ の物理定数を 1 ($$ 4\pi \epsilon_0 $$ も 1)とするハートリー原子単位系 $$
\hspace{-200px}1 E_h \,= \frac{e^2}{a_0} \;(\overset{hartree}{=} 1 \left[hartree\right]),\quad\quad \big( a_0 = \frac{\hbar^2}{m_e e^2} \;(\overset{hartree}{=} 1 \left[a.u.\right]) \big)
$$  

[^2]: 原子核の質量が, 電子のそれに比べて非常に大きいことを利用した近似

[^3]: 解きたい問題の Hamiltonian $$ H $$ が, $$
\hspace{-200px}H = H_0 + V $$ と書かれ, $$ H_0 $$については解かれている. すなわち, $$
\hspace{-200px}H_0\psi_n^{(0)} = E_n^{(0)}\psi_n^{(0)} $$ の解 $$ \psi_n^{(0)} $$ と $$ E_n^{(0)} $$ が既知であるとするとき, 元の問題 $$
\hspace{-200px}H_0\psi_n = E_n\psi_n $$ の解は, 次のように $$ \psi_n^{(0)} $$ と $$ E_n^{(0)} $$ で表せる. $$
\newcommand{\bracket}[3]{\left\langle #1 \middle| #2 \middle| #3 \right\rangle}
\hspace{-150px}\begin{align}
E_n &= E_n^{(0)} +  \bracket{\psi_n^{(0)}}{V}{\psi_n^{(0)}} + \sum_{k \neq n}\frac{\lvert \bracket{\psi_n^{(0)}}{V}{\psi_k^{(0)}}\rvert^2}{E_n^{(0)}-E_k^{(0)}} + \cdots  \\
\psi_n &= \psi_n^{(0)} + \sum_{k \neq n} \frac{\bracket{\psi_n^{(0)}}{V}{\psi_k^{(0)}}}{E_n^{(0)}-E_k^{(0)}} + \cdots
\end{align}
$$ ただし, $$ E_n^{(0)} $$ に縮退はないものとする. （縮退がある場合の摂動法は, もう少しだけ工夫が必要. ）右辺の第 2 項までを考慮する近似を１ 次の摂動法, 第 3 項まで入れたのを 2 次の摂動法と呼ぶ. 摂動法は非摂動ハミルトニアンの厳密解が知られていないときは適用できない.  
